...
| Console misconfigured at startup | The startup check runs first in the app's lifespan, before the inventory scheduler or the MCP server start, logs one line, and raises; the app does not start with the console enabled but unusable and nothing is left running. Flag off: none of this runs. |
