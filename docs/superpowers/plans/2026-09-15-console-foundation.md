...
**Architecture:** The console is its own repository, `seller-console`, and its own container: a small FastAPI app with Jinja2 pages and HTMX partials. It talks to the agent only over the agent's REST API, using one server-held operator key and an `X-Console-User` header. Accounts, sessions, and rate-limit records live in the console's own SQLite store. The runtime never imports the agent; tests install the agent package at a pinned version and run the console against the real API in-process.
...
- Pinned agent version: the fork's `ui/dev` at `701ce06` today, because the `me` route lands there first (Task 1). After Task 2 merges, the pin moves to that merge commit; after the upstream merge, to the upstream commit. `contract/AGENT_VERSION`, the dev dependency in `pyproject.toml`, and the build context in `compose.yml` always carry the same value.
...
| `src/seller_console/store.py` | `Store`: SQLite key-value store with TTL |
| `src/seller_console/accounts.py` | password hashing, account records, credential check |
| `src/seller_console/auth.py` | sessions, CSRF, rate limit, `Operator`, `ConsoleState`, `current_operator` |
...
| `Dockerfile`, `compose.yml`, `.github/workflows/ci.yml`, `README.md`, `.env.example` | build, run, CI, docs |

Delivery: Task 0 is console PR 0 (scaffold). Tasks 1 and 2 are agent PR 1 (`me` route, fork then upstream). Tasks 3 to 11 are console PR 2 (store, accounts, CLI, sessions, login, shell, startup). Task 12 is console PR 3 (landing page). Tasks 13 and 14 are console PR 4 (container, compose, README, smoke).
...
dev = [
    ...
    "ad_seller_system @ git+https://github.com/numarasSigmaSoftware/seller-agent.git@701ce06",
]
...
EOF
```
Expected: `75 paths` (76 once Task 1 lands and the pin moves).
...
## Task 1: `GET /auth/api-keys/me` on the agent

Runs in the agent worktree on `ui/dev`. Unchanged from the first plan.

**Files:**
- Modify: `src/ad_seller/interfaces/api/deps.py` (after `_require_operator_api_key_record`)
- Modify: `src/ad_seller/interfaces/api/routers/admin.py` (before `get_api_key_details`)
...
```
Expected: all passed. Open agent PR 1 against `ui/dev` on the fork, then upstream. **After it merges into `ui/dev`, move the console's pin**: set `contract/AGENT_VERSION`, the dev dependency, and the compose build context to the merge commit, `uv lock`, regenerate `contract/openapi.json` with the Task 0 snippet, run `tests/test_contract.py`, and commit `chore: pin the agent at <sha> (me route)`.
...
class ConsoleConfig(BaseSettings):
    ...
    console_db_url: str = "sqlite:///data/console.db"
...
"""The console's key-value store: accounts, sessions, and rate-limit records.

Same shape as the agent's generic storage (get, set with TTL, delete, keys by
pattern) so a Redis implementation can replace it for multi-instance deployments.
"""
...
```
Expected: `6 passed`. Revert check: drop the `expires_at` comparison in `get`: `test_ttl_expires` must fail. Restore.
...
async def test_reset_password(store):
    ...
    assert (await accounts.get_account(store, "nicolas")).credentials_changed_at > created.credentials_changed_at
    assert await accounts.check_credentials(store, "nicolas", "correct horse battery") is None
...
account routes. ``credentials_changed_at`` is bumped by every disable, password
reset, or role change; sessions issued before it are dead (see auth.py).
"""
...
async def check_credentials(store: Store, username: str, password: str) -> Optional[Account]:
    ...
    record = await store.get(ACCOUNT_PREFIX + username) if username else None
    ...
    if record is None or not ok or record["disabled"]:
        return None
    if needs_rehash:
        record["password_hash"] = hash_password(password)  # not a credential change
...
```
Expected: `11 passed` in about 15 seconds (scrypt at N=2^17 costs about a third of a second per call, which is the point). Revert checks: `hmac.compare_digest(...)` → `True`: the first and `test_check_credentials` must fail; `_LOG_N = 14`: the first and the rehash test must fail. Restore.
...
async def test_session_round_trip(store):
    token, expires_at = await auth.create_session(store, "nicolas", "operator", ttl_hours=1)
    assert len(token) >= 40
...
async def test_failure_counters(store):
    assert await auth.failures(store, "nicolas", "10.0.0.1") == 0
    for _ in range(3):
        await auth.record_failure(store, "nicolas", "10.0.0.1")
    assert await auth.failures(store, "nicolas", "10.0.0.1") == 3
    ...
    await auth.clear_failures(store, "nicolas", "10.0.0.1")
    assert await auth.failures(store, "nicolas", "10.0.0.1") == 0


async def test_concurrent_failures_are_all_counted(store):
    await asyncio.gather(*(auth.record_failure(store, "nicolas", "10.0.0.1") for _ in range(10)))
...
async def test_valid_cookie_resolves_operator(store):
    await accounts.create_account(store, "nicolas", "correct horse battery")
    app = _app_with_operator_route(_config(), store)
    token, _ = await auth.create_session(store, "nicolas", "operator", ttl_hours=1)
...
async def test_deleted_account_kills_an_issued_session(store):
    app = _app_with_operator_route(_config(), store)
    token, _ = await auth.create_session(store, "ghost", "operator", ttl_hours=1)
    assert (await _client(app).get("/console/whoami", cookies={auth.SESSION_COOKIE: token})).status_code == 303
...
async def test_sso_requires_header_and_ignores_cookies(store):
    ...
    token, _ = await auth.create_session(store, "nicolas@example.com", "operator", ttl_hours=1)
...
async def create_session(store: Store, username: str, role: str, ttl_hours: int) -> tuple[str, datetime]:
    token = secrets.token_urlsafe(32)
    created = _now()
    expires_at = created + timedelta(hours=ttl_hours)
    await store.set(
        SESSION_PREFIX + token,
        {"username": username, "role": role, "created_at": created.isoformat(), "expires_at": expires_at.isoformat()},
        ttl=ttl_hours * 3600,
    )
...
async def failures(store: Store, username: str, address: str) -> int:
    """One TTL key per failure, counted; never read-modify-written."""
    return max(len(await store.keys(scope + "*")) for scope in _rate_scopes(username, address))


async def record_failure(store: Store, username: str, address: str) -> None:
...
async def clear_failures(store: Store, username: str, address: str) -> None:
    for scope in _rate_scopes(username, address):
        for key in await store.keys(scope + "*"):
...
async def current_operator(request: Request) -> Operator:
    ...
    if session is not None and (
        account is None
        or account.disabled
        or datetime.fromisoformat(session["created_at"]) < datetime.fromisoformat(account.credentials_changed_at)
    ):
...
```
Expected: `16 passed`. Revert checks: `safe_next` → `return value or home`: the safe-next test must fail; drop the `is_htmx` branch: the HTMX redirect test must fail; delete the `from_trusted_proxy` check: the untrusted-address test must fail; delete the account-reread block: the three `*_kills_an_issued_session` tests must fail. Restore each.
...
transport over the pinned agent app in tests. Nothing else in the console imports
httpx. Every body is validated into a console-owned model; anything else degrades
...
class ConsoleApi:
    def __init__(self, transport: httpx.AsyncBaseTransport, base_url: str, operator_api_key: str, timeout: float = 2.0) -> None:
        # No owned connection: a client is opened per call and closed with it.
    ...
    async def _get(self, path: str, user: str, params: Optional[dict] = None) -> Any:
        ...
        try:
            async with httpx.AsyncClient(transport=self._transport, base_url=self._base_url, follow_redirects=False) as client:
...
```
Expected: `23 passed`. Revert checks: remove the `X-Console-User` header: the header test must fail; make `_parse` return `body`: the malformed-body test must fail. Restore.
...
def test_htmx_is_vendored_and_pinned():
    head = (CONSOLE / "static" / "htmx.min.js").read_text(errors="ignore")[:400]
    assert "htmx" in head and "2.0.4" in head
...
@pytest.fixture
def console_app(agent_app, config, store):
    transport = ASGITransport(app=agent_app, raise_app_exceptions=False)
    return create_app(config, transport=transport, store=store)
...
async def login(client: httpx.AsyncClient, username="nicolas", password="correct horse battery"):
    ...
    return await client.post("/login", data={"username": username, "password": password, "csrf": csrf, "next": "/"})
```
...
async def test_session_cookie_is_secure_behind_https_proxy(client, account):
    ...
    response = await client.post(
        "/login",
        data={"username": "nicolas", "password": "correct horse battery", "csrf": csrf, "next": "/"},
...
async def test_missing_csrf_is_400(client, account):
    ...
    response = await client.post("/login", data={"username": "nicolas", "password": "correct horse battery", "csrf": "bogus", "next": "/"})
...
async def test_sixth_failure_is_rate_limited(client, account, store):
    ...
    await auth.clear_failures(store, "nicolas", "127.0.0.1")
...
async def test_logout_deletes_session_and_clears_cookie(client, account, store):
    ...
    response = await client.post("/logout", data={"csrf": client.cookies["console_csrf"]})
    assert response.status_code == 303
...
async def test_every_post_route_requires_csrf(console_app):
    ...
    posts = [r for r in console_app.routes if isinstance(r, APIRoute) and "POST" in r.methods]
    assert posts
    for route in posts:
...
async def test_pages_issue_the_csrf_cookie_in_sso_mode(sso_client, store):
    from seller_console import accounts
...
def _render(request: Request, name: str, status_code: int = 200, **context: Any) -> HTMLResponse:
    """Every rendered page carries the CSRF token and (re)issues its cookie; require_csrf checks it."""
    ...
    response.set_cookie(auth.CSRF_COOKIE, token, httponly=True, samesite="lax", secure=auth.is_secure(request), path=auth.cookie_path(request))
    return response
...
async def login_submit(request: Request, username: str = Form(""), password: str = Form(""), next: str = Form("")) -> Any:
    ...
    if await auth.failures(state.store, username, address) >= auth.RATE_LIMIT_MAX:
    ...
    if account is None:
        await auth.record_failure(state.store, username, address)
    ...
    await auth.clear_failures(state.store, username, address)
    token, _ = await auth.create_session(state.store, account.username, account.role, state.config.console_session_ttl_hours)
...
The landing page and the partial are added in Task 12; `Awaitable`, the card models, `datetime`, and `Operator` are imported now so Task 12 only appends. Until then ruff reports them unused: add `# noqa: F401` to those two import lines and remove it in Task 12.
...
def create_app(
    ...
    *,
    ...
    api = ConsoleApi(
        ...
        config.console_operator_api_key,
...
The ASGI transport does not run the lifespan, which is why `test_healthz_is_503_until_startup_check_passes` flips `ready` by hand and why the lifespan tests in Task 11 enter it explicitly with a fresh store.
...
```
Expected: `test_login.py`: 17 passed; everything else still green. Revert checks: remove `dependencies=[Depends(require_csrf)]` from `login_submit`: the CSRF tests must fail; change `>= auth.RATE_LIMIT_MAX` to `> 100`: the rate-limit test must fail. Restore.
...
from seller_console.config import ConsoleConfig
from seller_console.main import create_app, lifespan, verify_console_key
from seller_console.store import Store
...
def test_missing_required_settings_refuse_to_build(monkeypatch):
    for name in ("SELLER_AGENT_URL", "CONSOLE_OPERATOR_API_KEY"):
        monkeypatch.delenv(name, raising=False)
    with pytest.raises(Exception):
        create_app(ConsoleConfig(_env_file=None))
...
```
Expected: `6 passed`. Revert checks: make `verify_console_key` return immediately: the revoked-key and buyer-key tests must fail; remove the retry branch: the retry test must fail. Restore.

This closes console PR 2: title `feat: console foundation (store, accounts, sessions, login, shell)`.

---

## Task 12: The landing page and the health partial
...
async def test_unreachable_agent_degrades_every_card_but_keeps_the_page(client, console_app, account):
    ...
    await login(client)
    console_app.state.console.api._transport = httpx.AsyncHTTPTransport()
    console_app.state.console.api._base_url = "http://127.0.0.1:9"
    page = await client.get("/")
    els = html_index(page.text)
    assert page.status_code == 200
...
This closes console PR 3: title `feat: landing page (Setup and health)`.

---

## Task 13: Container, compose, README

**Files:**
- Create: `Dockerfile`, `compose.yml`, `.env.example`, `README.md`
...
`uv lock` must have been run (Task 0) so `uv.lock` exists. The `--no-dev` sync keeps the agent package out of the image: the runtime never needs it.
...
```yaml
# compose.yml — the console next to the pinned agent. Set the variables in .env (see .env.example).
services:
  app:
    build:
      context: https://github.com/numarasSigmaSoftware/seller-agent.git#701ce06
  ...
  console:
    ...
    environment:
      ...
      CONSOLE_DB_URL: sqlite:////data/console.db
    env_file: [.env]
    ports: ["8080:8080"]
...
The git build context is what keeps the compose file self-contained; the commit after `#` is the same value as `contract/AGENT_VERSION`, and a test below checks that. To add the console to an existing agent stack instead, copy only the `console` service and the `console-data` volume into that stack's compose file.

- [ ] **Step 3: `.env.example`**

```bash
# Agent (see the seller-agent repository for its own variables)
# ANTHROPIC_API_KEY=
...
2. Mint the console's operator key on the agent and put it in `.env`:
   ```bash
...
| `CONSOLE_OPERATOR_API_KEY` | required | operator key for every API call |
| `CONSOLE_DB_URL` | `sqlite:///data/console.db` | the console's own store |
...
- [ ] **Step 5: Add the pin check and commit**
...
git add Dockerfile compose.yml .env.example README.md tests/test_contract.py
git -c user.name="numaras" -c user.email="nicolas.umaras@sigma.software" commit -m "feat: container image, compose stack with the pinned agent, README"
```
Expected: `4 passed`; the image builds.
...
```bash
cp .env.example .env
# 1. a bad key must stop the console container
sed -i.bak 's/^CONSOLE_OPERATOR_API_KEY=.*/CONSOLE_OPERATOR_API_KEY=ask_live_bogus/' .env
docker compose up -d --build app postgres redis
docker compose up --build --abort-on-container-exit console; echo "console exit=$?"
docker compose logs console 2>&1 | grep -c "was rejected by the agent"
# 2. mint the real key and an account
KEY=$(docker compose run --rm app ad-seller create-operator-key --label console --quiet)
sed -i.bak "s|^CONSOLE_OPERATOR_API_KEY=.*|CONSOLE_OPERATOR_API_KEY=$KEY|" .env
...
```
Expected: a non-zero console exit and `1` for the rejected-key line in step 1; `200`, `200`, `200` in step 3. Leave the stack running for step 3, then `docker compose down -v`.

- [ ] **Step 3: Manual browser smoke test (required before PR 3 merges)**

With the stack from step 2 running, in a browser, confirm each and tick them in the PR body:
...
```bash
docker compose down -v && rm -f .env .env.bak
```

This closes console PR 4: title `feat: compose stack, README, and smoke`.

---

## Self-review against the spec

- §3 repository, package, contract, CLI: Tasks 0, 5, 6, 9, 10, 13. The runtime-never-imports-the-agent rule and the drift test: Task 0; the compose pin check: Task 13.
- §4 store, accounts, login, sessions, `current_operator`, console key: Tasks 4, 5, 7, 10, 11; startup retry with the compose ordering: Tasks 11 and 13.
- §5 pages and palette: Tasks 9 and 12.
- §6 client with injected transport, per-call client, typed responses, connection errors: Task 8.
- §7 error handling including "agent unreachable" and `/healthz`: Tasks 8, 10, 12.
- §8 testing: real store, real pinned agent, boundary-forced failures, HTML by id, security and structural and wiring tests, compose smoke, browser decision: every task, Task 14.
- §9 configuration, deployment, README: Tasks 3 and 13.
- §10 delivery: PR boundaries after Tasks 0, 2, 11, 12, 13.
